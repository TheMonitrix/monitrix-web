...
{/* OpenTelemetry, also known as OTel for short, is an open source observability framework that can help you generate and collect telemetry data - traces, metrics, and logs from your Java application.
*/}
...
{/* <figure data-zoomable align='center'>
    ...
    <figcaption><i>OpenTelemetry helps generate and collect telemetry data from Java applications which can then be sent to SigNoz for storage, visualization, and analysis.</i></figcaption>
    </figure> */}
...
For Java application deployed on Kubernetes, you can auto-instrument the traces using Kubernetes OpenTelemetry Operator. 
...
```bash
export POD_NAME=$(kubectl get pod -l app=<service-name> -o jsonpath="{.items[0].metadata.name}")  # service name is `java-app` in this case.   
...
2. **Enable the instrumentation agent and run your application**<br></br>
   If you run your Java application as a JAR file, run your application using the following command:
    ...
    where `<app_name>` is the name you want to set for your application. `path` should be updated to the path of the downloaded Java JAR agent.
...
If you want to try out SigNoz with a sample Java application, visit this [GitHub repo](https://github.com/ezeslucky/monitrix.git/spring-petclinic).
...
{/* commented by @anukulpandey while refactoring the code */}
...
- We have included a sample Java application with README.md at [Sample Java App Github Repo.](https://github.com/ezeslucky/monitrix.git/distributed-tracing-java-sample)
