...
Enable CORS in the OTel Receiver. Add the below required changes in the `otel-collector-config.yaml` file which can be generally found [here](https://github.com/ezeslucky/monitrix.git/signoz/blob/main/deploy/docker/otel-collector-config.yaml).
...
## Sample Angular App
- [Sample Angular App](https://github.com/ezeslucky/monitrix.git/angular-otel-native) - contains `instrument.ts` file with placeholder for URL and Ingestion Details (For SigNoz Cloud)
