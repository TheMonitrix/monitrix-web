...
In case you have OtelCollector Agent in different machine, replace localhost:4317 with `<IP Address of the Machine>:4317`.
...
**Step 3.** Dockerize and Build your application
...
Here's a [working example](https://github.com/open-telemetry/opentelemetry-python/tree/main/docs/examples/fork-process-model) configured with a gunicorn server with `post_fork` hook.
...
- Hypercorn is not supported. There is no workaround for this. Please follow [this issue](https://github.com/pgjones/hypercorn/issues/215).
