...
You can auto-instrument sending traces from the Django application using one of the following methods:
...
For Python Django application deployed on Kubernetes, you can auto-instrument the traces using Kubernetes OpenTelemetry Operator. 
...
Here are the steps you need to follow to auto-instrument Django application using OTel Operator:
...
**Step 5.** Auto-instrument your Django app with OpenTelemetry

Create `deployment.yaml` file for your Django application as follows:
  ...
  template:
    metadata:
      ...
      annotations:
        ...
        resource.opentelemetry.io/service.name: "django-app"
    spec:
      containers:
      - name: app
...
This helps in auto-instrumenting the traces from the Django application.
...
With this, the auto-instrumentation of traces for Django application is ready.
...
```bash
export POD_NAME=$(kubectl get pod -l app=<service-name> -o jsonpath="{.items[0].metadata.name}")  # service name is `django-app` in this case.   
...
For Python Django application deployed on Kubernetes, you need to install OTel Collector agent in your k8s infra to collect and send traces to SigNoz Cloud. You can find the instructions to install OTel Collector agent [here](https://signoz.io/docs/tutorial/kubernetes-infra-metrics/).
...
- We have included a sample django application with README.md at [Sample django App Github Repo.](https://github.com/ezeslucky/monitrix.git/sample-django)
