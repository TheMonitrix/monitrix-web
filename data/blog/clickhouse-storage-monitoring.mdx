...
keywords: [ClickHouse database,Open Source,Open source database,OLAP databases,kafka,Druid]
---
...
In our [latest release](https://github.com/ezeslucky/monitrix.git/signoz) `v0.3.1` , we launched support for ClickHouse as storage backend for SigNoz. ClickHouse is a column-oriented database management system (DBMS) for online analytical processing of queries (OLAP). Now when you install SigNoz, you will have the option to choose between ClickHouse or Kafka + Druid as a database system.
...
As an open-source software, we build what our community wants. We listened to [demands](https://github.com/ezeslucky/monitrix.git/signoz/issues/22) from our community for supporting ClickHouse as a storage backend.
