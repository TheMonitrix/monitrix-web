...
4. Press `Import JSON`.
5. Paste the JSON from the following <a href = "https://github.com/ezeslucky/monitrix.git/dashboards/blob/main/k8s-infra-metrics/k8s-events-receiver.json" rel="noopener noreferrer nofollow" target="_blank" >github repo</a>. The JSON contains all the panels and their layout order.
6. Press `Load JSON`.
