...
keywords: [redis,redis monitoring,redis performance metrics,redis opentelemetry,opentelemetry redis,signoz,signoz apm]
---
...
You can have a look at the `otel-collector-config.yaml` file on our [GitHub repo](https://github.com/ezeslucky/monitrix.git/signoz/blob/main/deploy/docker/otel-collector-config.yaml). The final config file along with Redis receiver looks like below:

<Admonition>
We have configured the `redis endpoint` for Mac for this demo. You will need to update the endpoint based on your environment.
</Admonition>
...
In the `deploy/docker` folder, run the following command at your terminal to stop Docker containers:
