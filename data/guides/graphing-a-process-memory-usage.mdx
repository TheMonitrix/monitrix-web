...
If the app and otel-contrib ran without error, all your metrics would be sent to your SigNoz cloud. The next step is to create a dashboard to view the metrics data. For SigNoz, we have shared some commonly used dashboard JSON files [here](https://github.com/ezeslucky/monitrix.git/dashboards). 
    
    For this example, we will use the [JVM Metrics By Service](https://github.com/ezeslucky/monitrix.git/dashboards/blob/main/jvm/jvm-metrics-by-service.json) JSON file. To know more about how to import & customize Dashboards in SigNoz check our documentation [here](https://signoz.io/docs/userguide/manage-dashboards/). 
